...
async function describeImage(imageDataUrl: string, context: 'logo' | 'background'): Promise<string> {
  ...
  const prompt = context === 'logo'
    ? `Describe this image in intricate detail for a text-to-image AI model. Be descriptive and concise. Focus on key elements, colors, shapes, and any text. This description will be used to recreate this design on a t-shirt mockup.`
...
export const generateMockup = async (
  ...
  switch (productType as ProductType) {
    ...
    case 'flat_lay':
      ...
      prompt = `Create an ultra-realistic 4K photo of a product mockup.
    ...
    default:
        prompt = `Create an ultra-realistic 4K photo of a ${params.color || 'white'} ${productType.replace(/_/g, ' ')} mockup, with a design about "${designSubject}" in a ${params.designStyle.replace(/_/g, ' ')} style. ${text ? `The design includes the text "${text}".` : ''} The product is the main focus, on a clean, slightly blurred background.`;
        break;
  }
